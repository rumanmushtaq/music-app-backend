...
import { CommonMessages, CheckoutMessages } from '../constants/message';
import { subscriptionCacheKey } from '../constants/cache';
...
export class CheckoutService {
  ...
  async createOrder(clerkId: string, email: string, planId: string) {
    try {
      ...
      throw this.toHttpException(error, CheckoutMessages.createOrderFailed);
    }
  }
  ...
  async setAddress(clerkId: string, orderId: string, countryCode: string, stateCode?: string | null) {
    try {
      ...
      throw this.toHttpException(error, CheckoutMessages.setAddressFailed(orderId));
    }
  }
  ...
  async pay(clerkId: string, email: string, orderId: string, paymentProvider: string, paymentToken: string) {
    try {
      ...
      throw this.toHttpException(error, CheckoutMessages.payFailed(orderId));
    }
  }
  ...
  async getOrder(clerkId: string, orderId: string) {
    try {
      ...
      throw this.toHttpException(error, CheckoutMessages.getOrderFailed(orderId));
    }
  }
